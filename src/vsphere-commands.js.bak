        ...
        robot.respond(/(cpus) (.*)/i, function(cpuMSG) {
          ...
          robot.respond(/(name) (.*)/i, function(nameMSG) {
            ...
            robot.respond(/(os) (.*)/i, function(guestMSG) {
              ...
              return guestMSG.send(responses[Math.floor(Math.random()* responses.length)]);
            });
            return nameMSG.send(responses[Math.floor(Math.random()* responses.length)]);
          });
          return cpuMSG.send(responses[Math.floor(Math.random()* responses.length)]);
        });
        return memMSG.send(responses[Math.floor(Math.random()* responses.length)]);
      });
      return msg.send("...and then?")
    });
